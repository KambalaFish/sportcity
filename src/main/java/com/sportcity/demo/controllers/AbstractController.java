...
public abstract class AbstractController <DTO extends AbstractDTO<ID>, ID extends Serializable>{
    ...
    public ResponseEntity<Collection<DTO>> getAllById(@RequestParam("id") Collection<ID> idCollection){
    ...
    @DeleteMapping("/{id}")
    public ResponseEntity<?> deletedById(@PathVariable ID id){
        getService().deleteById(id);
        return ResponseEntity.ok().build();
    }

    @DeleteMapping("/collection")
    public ResponseEntity<?> deleteAllById(@RequestParam("id") Collection<ID> idCollection){
